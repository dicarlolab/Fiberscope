...
const int CMD_IS_MOTOR_MOVING = 22; 
const int CMD_ABORT_MOVEMENT = 23; 
...
private:
  ...
  int EEPROMaddress;
  float singleEncoderStepUm, microManipulatorFullRevolutionInUm,speedUmSec ;

  long targetPosition;
  long numMotorSteps; 
  long reachedPosition;
  int num_attempts;
  long num_steps_performed;
};
  ...
  EEPROMaddress(_EEPROMaddress),  encoderStepsFullRevolution(EncoderStepsFullRevolution), motorStepsFullRevolution(MotorStepsFullRevolution), microManipulatorFullRevolutionInUm(MicroManipulatorFullRevolutionInUm)
{
  ...
  bool myStepperClass::isMoving()
  {
    return motorFSM > 0;
  }
  
  void myStepperClass::abortMovement()
  {
    motorFSM = 6;
  }
...
void myStepperClass::runFSM()
{
  ...
  } else if (motorFSM == 2)
  {
      ...
      bool downDirection = targetPosition > currPos;
      initializMove(downDirection);
      long numEncoderSteps = abs(currPos - targetPosition);
      numMotorSteps = numEncoderSteps * motorStepsFullRevolution/ (float)encoderStepsFullRevolution;
  ...
  } else if (  motorFSM==3)
  {
    moveIterationMs();
    num_steps_performed++;
    if (num_steps_performed >= numMotorSteps)
    {
      motorFSM = 5;
    }
  } else if (  motorFSM==4)
  ...
  }else if (  motorFSM==5)
  {
    ...
    num_attempts++;
    if ( (abs(reachedPosition - targetPosition) <= accuracy_steps) || (num_attempts > max_attempts) )
    {
      motorFSM = 6;
    } else
    {
        motorFSM = 2; // another go at it...
...
long  myStepperClass::setAbsolutePositionSteps(long newPosition)
{
   ...
   motorFSM = 1;
   return getMotorPositionInEncoderSteps();
}
