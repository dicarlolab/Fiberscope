...
% Copyright 2013 by Timothy E. Holy
  ...
  for i = 1:numel(isnum)
    ...
    varname = ['/' varargin{i}];
    if prod(size(vars{i}))<4e9
        h5create(filename, varname, size(vars{i}), 'DataType', class(vars{i}),'ChunkSize',size(vars{i}));
    else
        h5create(filename, varname, size(vars{i}), 'DataType', class(vars{i}),'ChunkSize',size(vars{i})/2);
    end
