...
#define ALP_PWM_OUTPUT			// oscillating output at 15.6kHz, GPIO9
//#define ALP_MAX_PULSEWIDTH
//#define ALP_LED_CONTINUOUS 0	// 0=red or 1=green
...
// General ALP API stuff //////////////////////////////////////////////////////
	...
	const long nFrames=2;
	CAlpFramesMovingSquare ImageData(nFrames, nDmdWidth, nDmdHeight);

	// Allocate a sequence and load data
	long const nPicTimeInus = 44; // (microseconds)
	long const TriggerInDelayInus = 0; // (microseconds)
	_tprintf( _T("Note: Sequence timing: PicTime=%i µs, i.e. Frame rate=%i fps\r\n"),
		nPicTimeInus, 1000000/nPicTimeInus );	// convert µs to Hz
...
#ifdef ALP_MAX_PULSEWIDTH
	VERIFY_ALP( AlpSeqTiming( AlpDevId, AlpSeqId, 0, nPicTimeInus, 0, nPicTimeInus-1, TriggerInDelayInus ) );
#else
	VERIFY_ALP( AlpSeqTiming( AlpDevId, AlpSeqId, 0, nPicTimeInus, 0, 0, TriggerInDelayInus ) );
#endif

	VERIFY_ALP( AlpProjControl( AlpDevId, ALP_PROJ_MODE, ALP_SLAVE ) );
	VERIFY_ALP( AlpDevControl( AlpDevId, ALP_TRIGGER_EDGE, ALP_EDGE_FALLING ) );

#ifdef ALP_PWM_OUTPUT
	VERIFY_ALP( AlpDevControl( AlpDevId, ALP_PWM_LEVEL, 10 ) );	// GPIO9: free-running clock output at 15.6kHz, duty cycle 10% (10:90)
#endif
...
// LED Initialization Stuff ///////////////////////////////////////////////////
	...
	for (long nLedBusNumber=0; nLedBusNumber<nMaxLedCount; nLedBusNumber++) {
		...
		long LedType;
		if(nLedBusNumber == 0) {
			LedType = ALP_HLD_PT120_RED;
		}
		else {
			LedType = ALP_HLD_PT120_GREEN;
		}
		...
		long nLedBrightness_Percent;
		if(nLedBusNumber == 0) {
			nLedBrightness_Percent = 100;
		}
		else {
			nLedBrightness_Percent = 100;
		}
...
#ifdef ALP_LED_CONTINUOUS
		AlpSynchGate.Period = 1;		// enable synch output
		AlpSynchGate.Gate[0] = 0;		// but do not send pulses, so the level is constant inverted Polarity
		if (ALP_LED_CONTINUOUS==nLedIndex)
			AlpSynchGate.Polarity = 0;		// LED on
		else
			AlpSynchGate.Polarity = 1;		// LED off
#else
