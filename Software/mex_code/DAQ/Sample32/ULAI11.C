   ...
   */
    ...
    while (Gain > 0)
        ...
        if (Gain >= 0)
			...
			if(HighResAD)
				{
					ULStat = cbAIn32 (BoardNum, Chan, Gain, &DataValue32, Options);
					ULStat = cbToEngUnits32(BoardNum, Gain, DataValue32, &HighResEngUnits);

					printf ("\nThe voltage on channel %d is %.5f ", Chan, HighResEngUnits);
				}
			else
				{
